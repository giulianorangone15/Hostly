// ============================================================================================
// 🔐 ESTADO GLOBAL DE AUTENTICACIÓN Y VARIABLES GENERALES
// ============================================================================================
...
// ============================================================================================
// 🏨 BASE DE DATOS DE PROPIEDADES (infoPropiedades)
// ============================================================================================
const infoPropiedades = {
    'habitacion-executive': {
        ...
        precios: { 1: 45000, 2: 56000, 3: 68000, 4: 80000, 5: 87000, 6: 98000, 10: 80000 },
        imagenes: [ 
...
// ============================================================================================
// 🗓️ CONFIGURACIÓN DE AJUSTES POR TEMPORADA Y PRECIOS
// ============================================================================================
const PORCENTAJE_AUMENTO_FERIADO = 0.40;   // 40% extra
const PORCENTAJE_AUMENTO_VACACIONES = 0.30; // 30% extra (Temporada alta)
...
function verificarSiEsVacaciones(mes, dia) {
    if (mes === 0 || mes === 1) return true; // Enero y Febrero
    if (mes === 6) return true;             // Julio
    return false;
}
...
// ============================================================================================
// 🌐 INTERFAZ Y CONTROLADORES DE LOGIN PRINCIPAL (HOSTLY)
// ============================================================================================
...
// ============================================================================================
// 🗓️ LÓGICA DEL CALENDARIO Y MODAL DETALLE
// ============================================================================================
function generarCalendarioInterno() {
    ...
    for (let d = 1; d <= totalDias; d++) {
        ...
        if (esFeriado) {
            styleColorMonto = 'color: #d93838; font-weight: 700;'; 
        } else if (esVacaciones) {
            styleColorMonto = 'color: #007a78; font-weight: 600;'; 
        }
...
// ============================================================================================
// 💬 SALIDAS EXTERNAS DE COMUNICACIÓN (WHATSAPP)
// ============================================================================================
...
function solicitarServicioPropietario() {
    const modal = document.getElementById('modal-publicar-propiedad');
...
function enviarPublicacionWhatsApp() {
    const titulo = document.getElementById('pub-titulo').value;
    const precio = document.getElementById('pub-precio').value;
    const capacidad = document.getElementById('pub-capacidad').value;
    const ubicacion = document.getElementById('pub-ubicacion').value;
    const detalles = document.getElementById('pub-detalles').value || "No especifica";

    const telefonoAdministrador = "5493541523006"; 

    const mensajeWhatsApp = `¡Hola Giuliano! Vengo desde la plataforma *Hostly* 🏨 y quiero dar de alta mi propiedad.\n\n` +
                            `📋 *DATOS DE LA PROPIEDAD:*\n` +
                            `• *Nombre:* ${titulo}\n` +
                            `• *Precio por noche:* $${precio} ARS\n` +
                            `• *Capacidad:* ${capacidad} personas\n` +
                            `• *Ubicación:* ${ubicacion}\n` +
                            `• *Servicios:* ${detalles}\n\n` +
                            `💳 Quedo a la espera para coordinar el pago y la activación del anuncio.`;

    const urlFinal = `https://wa.me/${telefonoAdministrador}?text=${encodeURIComponent(mensajeWhatsApp)}`;

    cerrarModalPublicar();
    document.getElementById('form-publicar-propiedad').reset();
    window.open(urlFinal, '_blank');
}

// ============================================================================================
// 🛒 RENDERIZADO Y CONTROL DE FILTROS DEL CATÁLOGO
// ============================================================================================
